...
export class JarvisAiService {
  ...
  private buildSystemPrompt(
    ...
    return [
      ...
      'Call get_knowledge_base(topic=units) for the full unit glossary and field definitions.',
      'Use tools to query or refine any detail before deciding. Call get_knowledge_base(topic) to refresh a section; get_platform_rules for bounds and final-window / 1× / deleverage rule text.',
      ...
      'CRITICAL: Respect final_window_ms, in_final_window, leveraged_mint_blocked, and at_risk_of_force_deleverage on every market and position.',
      'Never open leverage > 1 when leveraged_mint_blocked is true. De-risk borrowed positions before the final window — keepers force-deleverage inside it.',
      ...
      'Respect platform_rules for leverage, margin, slippage, final_window_ms, and the rules text fields (final_window_rules, one_x_leverage_rules, force_deleverage_rules).',
